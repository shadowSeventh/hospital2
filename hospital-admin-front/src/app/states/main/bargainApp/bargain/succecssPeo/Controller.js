...
class Controller {
    constructor(_$scope,
        ...
        $scope.pageChange = function (currpage) {
            $http({
                method: "GET",
                url: conf.apiPath + "/bargainApp/" + $stateParams.bargainAppId + '/bargain/admin/' + $stateParams.id + '/getSuccessUser',
                params: {
                    page:currpage,
                    size:conf.pageSize,
        ...
        $scope.friendsList = function (data) {
            if(data.payStatus == false){
                ...
                $mdDialog.show({
                    ...
                    controller: ['$http', '$stateParams', '$mdDialog', '$rootScope', function ($http, $stateParams, $mdDialog, $rootScope) {
                        ...
                        vmd.getData = function () {
                            $http({
                                method: "GET",
                                url:conf.apiPath + "/bargainApp/" + $stateParams.bargainAppId + '/bargain/admin/' + $stateParams.id + '/user/' + data.userId  + '/order/show',
                                // url: conf.apiPath + "/bargainApp/" + $stateParams.bargainAppId + '/bargain/admin/' + $stateParams.id + '/user/' + data.userId + '/order/takeOff',
                                params: {},
                                headers: {
                                    // 'Authorization': 'Bearer ' + loginService.getAccessToken()
                                }
                ...
                $state.go("main.bargainApp.bargain.priceList",{id:$stateParams.id,userId:data.userId},{reload: true})
            }
        };
