...
class Controller {
    constructor(_$scope,
        ...
        $scope.pageChange = function (currpage) {
            $http({
                method: "GET",
                url: conf.apiPath + "/bargainApp/" + $stateParams.bargainAppId + '/bargain/admin/' + $stateParams.id + '/getBargainRecord',
                params: {
            ...
            $http({
                method: "GET",
                url: conf.apiPath + "/bargainApp/" + $stateParams.bargainAppId + '/bargain/admin/' + $stateParams.id + '/getNum',
                params: {},
                headers: {
                    // 'Authorization': 'Bearer ' + loginService.getAccessToken()
                }
            ...
            $mdDialog.show({
                ...
                controller: ['$http', '$stateParams', '$mdDialog', '$rootScope', function ($http, $stateParams, $mdDialog, $rootScope) {
                    ...
                    vmd.getFriendList = function (currpage) {
                        $http({
                            method: "GET",
                            url: conf.apiPath + "/bargainApp/" + $stateParams.bargainAppId + '/bargain/admin/bargainRecord/' + vmd.data.id + '/userId/' + vmd.data.userId + '/getBargainHelpUser',
                            params: {
                    ...
                    //     $http({
                    //         method: 'GET',
                    //         url: conf.apiPath + "/bargainApp/" + $scope.bargainAppId + '/activity/' + $stateParams.activityId + '/voteRecord/' + $stateParams.id,
                    //         params: {
                    //             page: vmd.currPage,
                    //             size: 6,
                    ...
                    //         },
                    //         headers: {
                    //             "bargainApp-Id": $scope.bargainAppId
                    //         }
                    //     }).then(function (resp) {
                    ...
                    vmd.goAddress = function () {
                        $mdDialog.cancel().then(function () {
                            $state.go('main.bargainApp.addAddress', {orderId: id}, {reload: true})
                        })
                    }
                }],
                controllerAs: "vmd"
